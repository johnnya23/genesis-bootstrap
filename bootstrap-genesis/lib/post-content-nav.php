...
add_action('genesis_entry_content', 'JMA_GBS_do_post_content_nav', 12);

// filter page links for correct bootstrap format
add_filter('wp_link_pages_link', 'JMA_GBS_wp_link_pages_link');

function JMA_GBS_wp_link_pages_link($link)
{
    if ($link && '<' !== $link[0]) {
...
function JMA_GBS_do_post_content_nav($attr)
{
    wp_link_pages(array(
        'before' => '<div class="jma-gbs-post-content-nav">'
