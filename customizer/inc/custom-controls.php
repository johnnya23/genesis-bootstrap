...
if (class_exists('WP_Customize_Control')) {
        ...
        public function render_content()
        {
		  ...
		  <div class="sortable_repeater_control">
				...
				<div class="sortable_repeater sortable">
					<div class="repeater">
						<input type="text" value="" class="repeater-input addr" placeholder="https://" /><span class="dashicons dashicons-sort"></span><a class="customize-control-sortable-repeater-delete" href="#"><span class="dashicons dashicons-no-alt"></span></a>
					</div>
				</div>
     ...
     * @link https://github.com/maddisondesigns
     */
    class JMA_GBS_Sortable_Repeater_Custom_Control extends Skyrocket_Custom_Control
    {
        /**
         * The type of control being rendered
         */
        public $type = 'jma_gbs_sortable_repeater';
        /**
        ...
        {
            ?>
		  <div class="sortable_repeater_control multi_field">
				<?php if (!empty($this->label)) {
                ?>
					<span class="customize-control-title"><?php echo esc_html($this->label); ?></span>
				<?php
                ...
                foreach ($this->choices as $key => $value) {
                    if (is_array($value)) {
                        ...
                        foreach ($value as $optgroupkey => $optgroupvalue) {
                            echo '<option value="' . esc_attr($optgroupkey) . '" ' . (in_array($optgroupkey, $this_value) ? 'selected="selected"' : '') . '>' . esc_attr($optgroupvalue) . '</option>';
                        }
                        echo '</optgroup>';
                    } else {
                        echo '<option value="' . esc_attr($key) . '" ' . selected(esc_attr($key), $this_value, false)  . '>' . esc_attr($value) . '</option>';
                    }
                } ?>
				</select>
