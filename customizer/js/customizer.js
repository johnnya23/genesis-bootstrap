jQuery(document).ready(function($) {
    ...
    $('.sortable_repeater_control').each(function() {
        ...
        var is_multi_field = $(this).hasClass('multi_field');
        if (!is_multi_field) {
    ...
    $('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
        ...
        var numItems = $(this).closest('.sortable_repeater.sortable').children().length;
        var $repeater = $(this).closest('.repeater');
        var $parentContainer = $(this).closest('.sortable_repeater_control');
        if (numItems > 1) {
            $repeater.slideUp('fast', function() {
                $repeater.remove();
                skyrocketGetAllInputs($parentContainer);
            });
        } else {
            if ($repeater.parents('multi_field').length) {
                $repeater.find('option').each(function() {
                    $(this).attr('select', '');
                });
            } else {
    ...
    $('.customize-control-sortable-repeater-add').click(function(event) {
        event.preventDefault();
        skyrocketAppendRow($(this).closest('.sortable_repeater_control'));
        skyrocketGetAllInputs($(this).closest('.sortable_repeater_control'));
    });
    ...
    function skyrocketAppendRow($element, defaultValue = '') {
        var newRow = '';
        if ($element.hasClass('multi_field')) {
            var $repeater = $element.find('.repeater');
            var $new_repeater = $repeater.last().clone();
            newRow = $new_repeater;
        } else {
            newRow = '<div class="repeater" style="display:none"><input type="text" value="' + defaultValue + '" class="repeater-input" placeholder="https://" /><span class="dashicons dashicons-sort"></span><a class="customize-control-sortable-repeater-delete" href="#"><span class="dashicons dashicons-no-alt"></span></a></div>';
        }
    ...
    function skyrocketGetAllInputs($element) {
        var $repeater = $element.find('.repeater');
        var inputValues = $repeater.map(function() {
            var multi_return = '';
            if ($element.hasClass('multi_field')) {
                $(this).find('option').each(function() {
                    if ($(this).attr('selected')) {
                        multi_return += $(this).val() + '|';
                    }
                });
                return multi_return.slice(0, -1);
            } else {
                return $(this).find('.repeater-input').val();
            }
        }).toArray();
